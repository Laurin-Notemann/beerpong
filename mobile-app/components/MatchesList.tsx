...
const groupIntoDays = (matches: Match[]) => {
  const dayjsMap = matches.reduce(
    (acc: Record<string, { matches: Match[]; title: string }>, obj: Match) => {
      const dayKey = dayjs(obj.date).format("YYYY-MM-DD");

      if (!acc[dayKey]) {
        acc[dayKey] = { matches: [], title: getDayName(dayjs(obj.date)) };
      }
            ...
            >
              <>
                ...
                <View style={{ flexDirection: "row", gap: 16 }}>
                  <Text
                    style={{ fontSize: 15, color: theme.color.text.tertiary }}
                  >
                    12:15
                  </Text>
                  <Text
                    style={{ fontSize: 15, color: theme.color.text.tertiary }}
                  >
                    {item.blueTeam.map((i) => i.name).join(", ") +
                      " - " +
                      item.redTeam.map((i) => i.name).join(", ")}
                  </Text>
                </View>
              </>
            </TouchableHighlight>

    ...
    const getCupPanGesture = (cup: Cup) =>
        Gesture.Pan()
            ...
            .onChange((event) => {
                ...
                const currentX =
                    draggedCup.value.pos.posX + (event.translationX ?? 0);
                const currentY =
                    draggedCup.value.pos.posY + (event.translationY ?? 0);
                ...
                for (const point of gridPoints) {
                    const distance = Math.sqrt(
                        (currentX - point.posX) ** 2 +
                            (currentY - point.posY) ** 2
                    );
                ...
                const stateDidntChange =
                    cup.pos.x === closestGridPoint.x &&
                    cup.pos.y === closestGridPoint.y;

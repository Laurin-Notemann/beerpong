...
public interface MatchRepository extends JpaRepository<Match, String> {
    ...
    @Query("SELECT DISTINCT m FROM Match m JOIN Team t ON t.match.id = m.id JOIN TeamMember tm ON tm.team.id = t.id WHERE m.season.id = :seasonId AND tm.player.profile.id = :profileId")
    List<Match> findBySeasonIdAndProfileId(@Param("seasonId") String seasonId, @Param("profileId") String profileId);
